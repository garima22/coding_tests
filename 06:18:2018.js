...
function nameMatch(knownAliases, name) {
  // Implement me
  var name_arr = splitName(name);
  var alias = '';
  
  for(var i=0; i<knownAliases.length; i++){
  
    var alias = knownAliases[i];
    var alias_arr = splitName(alias);
    // 1. Exact match
    ...
    // Middle name missing
    //console.log(checkForMiddleNameMissing(name_arr, alias));
    if (checkForMiddleNameMissing(name_arr, alias_arr)){
      return true;
    }
    
    
    // Middle initial matches middle name
    if (checkForMiddleInitialMatch(name_arr, alias_arr)){
      return true;
    }
    
    // Transposed
    if (checkForTransposed(name_arr, alias_arr)){
      return true;
    }
...
function checkForTransposed(name_arr, alias_arr){

  if (alias_arr['fname'] == name_arr['mname'] && alias_arr['mname'] == name_arr['fname'] && alias_arr['lname'] == name_arr['lname']){
    return true;
  }
  
  // Middle name missing on name
  var transposed_name_arr = [];
  transposed_name_arr['fname'] = '';
  transposed_name_arr['mname'] = name_arr['fname'];
  transposed_name_arr['lname'] = name_arr['lname'];
  if (checkForMiddleNameMissing(transposed_name_arr, alias_arr)){
    return true;
  }
...
function checkForMiddleNameMissing(name_arr, alias_arr){
    
  if (alias_arr['mname'] == '' || name_arr['mname'] == ''){
    if (alias_arr['fname'] == name_arr['fname'] && alias_arr['lname'] == name_arr['lname']){
      return true;
    } 
  }
  return false;
...
function checkForMiddleInitialMatch(name_arr, alias_arr){
  
  
  // Middle initial in alias
  if (alias_arr['mname'].length == 1){
    if (alias_arr['fname'] == name_arr['fname'] && alias_arr['lname'] == name_arr['lname'] && alias_arr['mname'] == name_arr['mname'].charAt(0)){
      return true;
    } 
  }
  else if (name_arr['mname'].length == 1){
    if (alias_arr['fname'] == name_arr['fname'] && alias_arr['lname'] == name_arr['lname'] && alias_arr['mname'].charAt(0) == name_arr['mname']){
      return true;
    } 
  }
...
function splitName(name){
  ...
  var name_arr = [];
  name_arr['fname'] = parts[0];
  if (parts.length == 2){
    name_arr['mname'] = '';
    name_arr['lname'] = parts[1];
  }
  else{
    name_arr['mname'] = parts[1];
    name_arr['lname'] = parts[2];
  }
  return name_arr;
}
